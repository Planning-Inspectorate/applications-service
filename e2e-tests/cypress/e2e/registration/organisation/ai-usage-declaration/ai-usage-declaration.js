...
import PO_AiUsageDeclaration from '../ai-usage-declaration/PageObjects/PO_AiUsageDeclaration';
...
const aiUsageDeclaration = new PO_AiUsageDeclaration();
...
Then('I am on the {string} page', (pageName) => {
	if (pageName.toLowerCase().includes('ai usage declaration')) {
		aiUsageDeclaration.assertOnPage('ai usage declaration organisation');
		return;
	}
...
When('user selects {string} radio option on AI usage declaration page', (radioChoice) => {
	aiUsageDeclaration.selectRadioOption(radioChoice);
});

And('I click on the continue button', () => {
	aiUsageDeclaration.clickAISaveAndContinue();
});

Then('the AI declaration page displays all required content', () => {
	aiUsageDeclaration.assertPageContent();
});

Then('an error message is displayed for AI declaration: {string}', (msg) => {
	aiUsageDeclaration.assertErrorMessages({ hashes: () => [{ ErrorMsg: msg }] });
});

Then('the AI declaration page displays correct Welsh content', () => {
	aiUsageDeclaration.assertWelshPageContent();
});

Given('I switch the language to Welsh', () => {
	cy.get('a[href*="?lang=cy"]').click();
	cy.waitForDemoDelay();
});
