	...
	plugins: [
		...
		new CopyPlugin({
			patterns: [
				...
				{
					from: path.resolve(nodeModulesPath, 'leaflet.markercluster', 'dist', 'MarkerCluster.css'),
					to: path.resolve(publicPath, 'stylesheets', 'MarkerCluster.css')
				},
				{
					from: path.resolve(
						nodeModulesPath,
					...
					to: path.resolve(publicPath, 'stylesheets', 'MarkerCluster.Default.css')
				}
			]
		})
	]
