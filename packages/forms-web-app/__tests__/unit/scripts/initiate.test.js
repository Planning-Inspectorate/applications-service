...
describe('scripts/initiate', () => {
	...
	test('Expect script tag to have defer attibute without a async attibute', () => {
	...
	test('Expect callback to not be executed', () => {
		expect(scriptFailNoSrc).toBeNull();
		expect(document.value.fail.noSrc).not.toBeDefined();
	});

	test('Expect script tag to be null', () => {
		expect(scriptFailNotArray).toBeNull();
	});

	test('Expect config object to not modify allScriptTags', () => {
