...
(function (Modules) {
	...
	AppStepNav.prototype.init = function () {
		...
		this.$module.stepNavSize = this.$module.classList.contains('app-step-nav--large')
			? 'Big'
			: 'Small';
		this.$module.rememberShownStep =
			!!this.$module.hasAttribute('data-remember') && this.$module.stepNavSize === 'Big';
	...
	AppStepNav.prototype.removeActiveStateFromAllButCurrent = function (activeLinks, current) {
		for (var i = 0; i < activeLinks.length; i++) {
			...
			if (
				thisel.querySelectorAll('.js-link')[0].getAttribute('data-position').toString() !==
				current.toString()
			) {
	...
	AppStepNav.prototype.bindToggleShowHideAllButton = function (stepNavTracker) {
		...
		this.$module.showOrHideAllButton.addEventListener('click', function (event) {
			...
			stepNavTracker.trackClick(
				...
				{
					label:
						(shouldShowAll ? that.$module.actions.showAllText : that.$module.actions.hideAllText) +
						': ' +
						that.$module.stepNavSize
				}
			);
