...
/**
 * Get filter label from i18n using namespace-specific translation
 * The i18n object should already have the namespace applied via a wrapper
 *
 * @param {Object} i18n - Internationalization object with t() method
 * @param {string} name - Filter name (e.g., 'region', 'sector', 'stage')
 * @returns {string} Translated filter label or name if not found
 */
const getFilterLabel = (i18n, name) => i18n.t(`filterLabels.${name}`) || name;
 ...
 * @param {number} filterItem.count - Number of projects with this filter
 * @param {string} filterItem.label - English label
 * @param {string} filterItem.label_cy - Welsh label
 * @param {string} filterItem.value - Filter value
 * @returns {Object} Filter item view model for checkbox accordion
 */
 ...
 * @param {string} filterGroup.name - Filter name (e.g., 'region')
 * @returns {Object} Filter group view model for checkbox accordion
 */
