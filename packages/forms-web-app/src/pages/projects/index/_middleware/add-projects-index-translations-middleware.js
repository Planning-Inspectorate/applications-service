...
const addProjectsIndexTranslationsMiddleware = (req, res, next) => {
	const { i18n } = req;
	const { language } = i18n;

	const translations = getTranslations(enTranslation, cyTranslation);

	i18n.addResourceBundle(language, projectsIndexI18nNamespace, translations[language]);

	next();
};

module.exports = { addProjectsIndexTranslationsMiddleware };
