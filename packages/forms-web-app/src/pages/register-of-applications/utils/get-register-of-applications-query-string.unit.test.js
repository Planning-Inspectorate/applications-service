	...
	describe('#getRegisterOfApplicationsQueryString', () => {
		describe('When getting the register of applications query string', () => {
			describe('and there are no query parameters', () => {
				...
				it('should return the default register of applications query string', () => {
					expect(registerOfApplicationsQueryString).toEqual(
						'?excludeNullDateOfSubmission=true&searchTerm=&sort=-DateOfDCOSubmission&stage=acceptance&stage=pre_examination&stage=examination&stage=recommendation&stage=decision&stage=post_decision&stage=withdrawn'
					);
				});
			});
			...
			describe('and there are query parameters', () => {
				...
				it('should return the default register of applications query string', () => {
					expect(registerOfApplicationsQueryString).toEqual(
						'?excludeNullDateOfSubmission=true&searchTerm=&sort=mock%20sort%20by&stage=acceptance&stage=pre_examination&stage=examination&stage=recommendation&stage=decision&stage=post_decision&stage=withdrawn'
					);
				});
			});
		});
